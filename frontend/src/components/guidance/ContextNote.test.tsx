...
describe("ContextNote", () => {
  ...
  it("uses canonical content and can be dismissed without completing work", async () => { render(<ContextNote topicId="dieu-huong-so-do" role="reader" />); expect(await screen.findByText("Tìm người và di chuyển trên sơ đồ")).toBeInTheDocument(); fireEvent.click(screen.getByRole("button", { name: "Đã hiểu" })); expect(screen.queryByText("Tìm người và di chuyển trên sơ đồ")).not.toBeInTheDocument(); });
