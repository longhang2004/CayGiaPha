...
function TreeListContent() {
  ...
  return (
    <main className="tree-list-page">
      ...
      <TreeListView
        trees={treesList}
        onAddTree={() => setIsCreateModalOpen(true)}
        onOpenTree={(treeId) => router.push(`/tree/${treeId}`)}
        onDeleteTree={handleDeleteTree}
        notice={error ? <div className="tree-list-page__error">{error}</div> : null}
        emptyGuidance={(
          <GuidanceChecklist
            role="owner"
            productState={{ treeOpened: false, personCount: 0, primitiveCount: 0, addressInspected: false, viewpointChanged: false }}
          />
        )}
      />
