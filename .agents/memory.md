...
- **Home Page and Search Toolbar Styling Fixes**: Constrained `.home-hero` page width to `70rem` centered. Removed inline styles from `SearchPanel.tsx` and moved them to `globals.css` with a responsive media query breakpoint (`@media (max-width: 1150px)`) to prevent wrapping issues when the sidebar is open.
