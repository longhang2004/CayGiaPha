...
- **Execution routing default**: Follow `.agents/execution-routing-harness.md` in Plan mode. Default to the lead/heavy model planning and implementing in the same session. Escalate to PO/BA or Designer only for unresolved product/design authority, and delegate to Medium/Light only when work is independent, contract-stable, verifiable, non-overlapping, and saves more effort than prompt/review/integration overhead.
